...
> **状态：v0.2.0 —— 预防型，不是检测型。** 卡死已定位到具体窗口与具体代码路径（见下），插件逻辑有
> **55 项测试**（55 通过 / 0 失败），并且已在**隔离的 `DSH_HOME`** 里装载并跑通接线（绝不拿正在运行的
> 实例做验证对象）。**但请谨慎理解下面的机制：代码级事实是核实的，而"重放导致卡死"这条因果链并未证明**
> —— 存在反例，写在 `docs/DESIGN.md` 的 *Open questions* 里。卡死本身未受控复现，冷启动首轮的代价也还
> 没有归因到任何具体监听器上。
...
> **⚠️ 先读这段：下面那条因果结论已被实测证伪。** 表格里的代码级事实是核实过的，但它们**不是**卡死的原因。
> `tools/measure-replay.js` 用**产品自己的 `TokenMeter._sync`** 在重建出的真实会话上跑了一遍：
> **整份 526,383 seq 的冷重放只要 47 毫秒** —— 而冷启动首轮实测 **16,008 毫秒**，更有永不返回的回合。
> 重放小了约 **340 倍**，不可能是原因。**真凶仍然未知**；仍然成立的是：冷启动首轮远慢于热轮、随会话规模增长、
> 且 `/compact` 是唯一被观测到的恢复手段。详见 `docs/DESIGN.md` 的 *Measured falsification*。
...
> **基于上面的证伪，这里必须说实话：插件并不能"吃掉"那份神秘代价。** `compactNow` 的第一步是 `measure()`，
> 而它是廉价的（整份会话 47 ms），所以载入时压缩**并不会**替你付掉那个慢监听器的账。插件真正交付的，
> 是把**唯一被观测到能恢复服务**的操作自动化：会话已经很大时在载入后压一次、回合之间随增长再压。
> 如果那个慢监听器的代价随**可见 surface** 增长，保持 surface 小就有用 —— **但这是假设，不是结论。**
...
| `tools/measure-replay.js` | 用**产品自己的 `TokenMeter._sync`** 在重建的真实会话上量冷重放耗时 —— 就是**证伪**上面那条因果的测量（526,383 seq 共 47 ms）。 |
...
- **已在隔离 `DSH_HOME` 验证，但不是对卡死本身的端到端验证。** 一次真实的 headless Harness 运行装载了本插件、
  触发了它的 `agent/status` 钩子、解析到 `ctx.get('compaction')`，并真的开出压缩事务
  （`compaction/start` 携带 `sourceCommandId: "session-rescue-guard-1"`）—— 复现两次。卡死本身**未**复现，
  且"回合前抢跑"的时序只有单测覆盖。详见 `docs/DESIGN.md` 的 *Verification performed*。
